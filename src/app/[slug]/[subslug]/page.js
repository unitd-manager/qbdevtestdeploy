...
export async function generateMetadata({ params }) {
  ...
  // Parent path is the slug parameter
  const parent_path = slug || '';
  
  // Try different slug combinations
  let data = await fetchPostBySlug(subslug);
  let pageType = 'post';
  let finalSlug = subslug;
  
  if (!data) {
    data = await fetchPostBySlug(`${slug}/${subslug}`);
    finalSlug = `${slug}/${subslug}`;
  }
  
  if (!data) {
    data = await fetchPageBySlug(subslug);
    pageType = 'page';
    finalSlug = subslug;
  }
  
  if (!data) {
    data = await fetchPageBySlug(`${slug}/${subslug}`);
    pageType = 'page';
    finalSlug = `${slug}/${subslug}`;
  }
...
export default async function SlugPage({ params }) {
  ...
  // Try different slug combinations
  let page_data = await fetchPostBySlug(subslug);
  let pageType = 'post';
  let finalSlug = subslug;
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPostBySlug(`${slug}/${subslug}`);
    finalSlug = `${slug}/${subslug}`;
  }
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPageBySlug(subslug);
    pageType = 'page';
    finalSlug = subslug;
  }
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPageBySlug(`${slug}/${subslug}`);
    pageType = 'page';
    finalSlug = `${slug}/${subslug}`;
  }
