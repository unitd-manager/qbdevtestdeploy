...
export async function generateMetadata({ params }) {
  ...
  const { slug, subslug, thirdslug } = resolvedParams;
  
  // Parent path includes both slug and subslug
  const parent_path = `${slug}/${subslug}`;
  
  // Try different slug combinations
  let data = await fetchPostBySlug(thirdslug);
  let pageType = 'post';
  let finalSlug = thirdslug;
  
  if (!data) {
    data = await fetchPostBySlug(`${slug}/${subslug}/${thirdslug}`);
    finalSlug = `${slug}/${subslug}/${thirdslug}`;
  }
  
  if (!data) {
    data = await fetchPageBySlug(thirdslug);
    pageType = 'page';
    finalSlug = thirdslug;
  }
  
  if (!data) {
    data = await fetchPageBySlug(`${slug}/${subslug}/${thirdslug}`);
    pageType = 'page';
    finalSlug = `${slug}/${subslug}/${thirdslug}`;
  }

  return generatePageMetadatas(finalSlug, pageType, parent_path, {});
}
...
export default async function SlugPage({ params }) {
  ...
  // Try different slug combinations
  let page_data = await fetchPostBySlug(thirdslug);
  let pageType = 'post';
  let finalSlug = thirdslug;
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPostBySlug(`${slug}/${subslug}/${thirdslug}`);
    finalSlug = `${slug}/${subslug}/${thirdslug}`;
  }
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPageBySlug(thirdslug);
    pageType = 'page';
    finalSlug = thirdslug;
  }
  
  if (!page_data || !page_data?.id) {
    page_data = await fetchPageBySlug(`${slug}/${subslug}/${thirdslug}`);
    pageType = 'page';
    finalSlug = `${slug}/${subslug}/${thirdslug}`;
  }

  const metadata = await generatePageMetadatas(finalSlug, pageType, parent_path, {});
            ...
            (await (async () => {
              ...
              return finalLayouts.map(item => (
                <DynamicComponent
                  ...
                  layout={item.layout}
                  data={{ ...item.data, pageId: page_data.id }} 
