'use client';
import { SectionTag, DivTag, H2Tag, PTag } from '../Common/HTMLTags';
...
export default function ContentHighlight({ data }) {
    ...
    return (
        <SectionTag className={`acf-section usecase-highlight-block ${isRight ? 'usecase-highlight-block-right' : 'usecase-highlight-block-left'} ${class_name}`} id={id}>
                    ...
                    <DivTag className={`row align-items-center ${isRight ? 'flex-row-reverse' : ''}`}>
                        ...
                        <DivTag className="col-md-6">
                            <DivTag className={`content-box text-${text_alignment}`}>
                                {usecase_category && <PTag className="category-name" dangerouslySetInnerHTML={{ __html: usecase_category }} />}
