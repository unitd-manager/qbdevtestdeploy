...
import { DivTag, H2Tag, H4Tag, PTag, SectionTag, SpanTag } from '../Common/HTMLTags';
...
export default function Sectionthree({ data }) {
  ...
  const services = data?.automation_edge_list_ || [];
  const main_title = data?.main_title;
  const description = data?.description;
  const cta_button = data?.cta_button;
  ...
  return (
    <SectionTag className="our-searvice-info">
      <DivTag className={`container our-searvice-data ${paddingClass}`}>
        ...
        <DivTag className={`row list-of-service ${inView ? 'animated-border' : ''}`} ref={sectionRef}>
                ...
                services.map((service, index) => (
                  ...
                  >
                    <DivTag className="serv-list-heading">
                      ...
                      <SpanTag className="right-arrow-iocn">
                    ...
                    <DivTag className="serv-list-desc" style={{ display: index === activeIndex ? 'block' : 'none' }}>
                      <DivTag className="serv-list-desc-info">
                        ...
                        <DivTag className="serv-desc">
                          <PTag dangerouslySetInnerHTML={{ __html: service.description }} />
                          {service.button?.url && (
                            <Link
                              href={service.button.url}
          ...
          <DivTag className="col-md-6 serv-right-info hide-for-small">
            <DivTag className="serv-media-right-info" >
              {services[activeIndex]?.image?.url && (
                <Image src={services[activeIndex].image.url} alt={services[activeIndex].title || ''} fill />
              )}
            </DivTag>
          </DivTag>
        </DivTag>
