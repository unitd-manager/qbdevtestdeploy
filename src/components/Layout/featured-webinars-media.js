...
import { DivTag, H4Tag, PTag, SectionTag } from '../Common/HTMLTags';
import Link from 'next/link';
import Image from 'next/image';
import { ArrowRight } from '../Common/Icons';

export default function FeaturedWebiner({ data }) {
    ...
    if (!data) return null;
    const { media_cards = [], class_name, id, padding } = data;
    ...
    return (
        <SectionTag className={`featured-webinars ${class_name}`} id={id}>
            <DivTag className="container-fluid">
                <DivTag className={`sub-section ${paddingClass}`}>
                    <DivTag className="row grid-row" ref={rowRef}>
                        {media_cards.map((row, rowIndex) => {
                            const colClass = rowIndex % 2 === 0 ? 'odd-col' : 'even-col';
                            return (
                                <DivTag
                                    key={rowIndex}
                                    className={`col-md-4 card-col ${colClass}`}
                                    ref={(el) => (cardRefs.current[rowIndex] = el)} // store ref in array
                                >
                                    <DivTag className="content-block">
                                        ...
                                        {row.category__tag && (
                                            <PTag
                                                className="tag"
                                                dangerouslySetInnerHTML={{ __html: row.category__tag }}
                                            />
                                        )}
